"""B6c reference-led yellow/white and three playful designs.
...
KINDS={'comicred','romance','emojiwhite','contrastpop'}
MODES={'comicred':{'normal','display'},'romance':{'normal','display'},'emojiwhite':{'normal','hand'},'contrastpop':{'normal','arc'}}
SYMBOLS={'star','heart','arrow','burst','exclaim'}

def validate(p):
    from template_scenes import texts,interval
    k=p.variant['scene_system']['kind'];ids={}
    if any(p.p.get(x) for x in ('scene_notes','scene_highlights','scene_title_surface','scene_identity','scene_accents','scene_media','scene_callouts')):raise ValueError('Unsupported B6c auxiliary layer')
    for g in p.p['scene_captions']:
        for e in g['phrases']:
            ident=e.get('id')
            if not isinstance(ident,str) or not ident or ident in ids:raise ValueError('B6c requires unique nonempty phrase ids')
            ids[ident]=e
            if e.get('mode','normal') not in MODES[k]:raise ValueError('Unsupported B6c phrase mode')
            if any(e.get(x) for x in ('pointer','underline','reveal','reveal_times')) or e.get('color','normal')!='normal' or any('fill' in r for r in e.get('runs',[])):raise ValueError('B6c colors/decorations are contract controlled')
    for e in p.p.get('scene_tags',[]):
        a,b=interval(e,p.p['duration']);q=ids.get(e.get('phrase_id'))
        if not q or not q['start']<=a<b<=q['end'] or e.get('text') not in texts(q):raise ValueError('B6c tag must quote an active phrase')
        if e.get('orientation','horizontal')!='horizontal' or e.get('side','right') not in ('left','right'):raise ValueError('B6c tags are horizontal and side anchored')
        symbol=e.get('symbol')
        if symbol is not None and (symbol not in SYMBOLS or k=='comicred'):raise ValueError('Unsupported semantic sticker symbol')
        if symbol and not str(e.get('symbol_reason','')).strip():raise ValueError('Sticker symbol requires explicit semantic reason')
        if any(x in e for x in ('asset','path','image','icon')):raise ValueError('B6c tags use original geometry, not undeclared sticker assets')
        if any(v in e for v in ('x','y')):
            from template_scenes import finite
            if not all(v in e and finite(e[v]) and 0<=e[v]<=1 for v in ('x','y')):raise ValueError('B6c explicit tag anchor needs normalized x/y pair')
...
def _metrics(p,runs,size,role,mode,maxw):
    ...
    for _ in range(5):
        glyphs=[];x=0
        for r in runs:
            rr=r.get('role',role);mult=1.08 if rr=='keyword' and k in ('romance','emojiwhite') else 1
        ...
        if size<initial*.70:raise ValueError('B6c text too long; split semantic phrases rather than shrink')
    raise ValueError('B6c text does not fit')
...
def line(p,runs,size,role='body',mode='normal',maxw=None):
    ...
    for x,text,f,rr,b in glyphs:
        ...
        if k=='comicred':
            fill='#FFF2E8' if rr=='keyword' else '#FFFFFF';edge='#B8231D';sw=1.5 if mode=='display' else .9
        elif k=='romance':
            fill='#F49AC2' if rr=='keyword' else '#FFF8F1';edge='#C56C8D';sw=1.8
        elif k=='emojiwhite':
            fill='#111111' if rr=='keyword' else '#FFFFFF';edge='#111111';sw=2.2
        else:
            fill={'title':'#FFB05A','keyword':'#FF6B3D','body':'#FFF4E7','sticker':'#FFB05A'}.get(rr,'#FFF4E7');edge='#111111';sw=2.6
        d.text((pos[0]+1.4*u,pos[1]+2*u),text,font=f,anchor='ls',fill=edge,stroke_width=max(1,round((sw+1.4)*u)),stroke_fill=edge)
        outer='#FFFFFF' if k in ('romance','emojiwhite') else edge
        d.text(pos,text,font=f,anchor='ls',fill=fill,stroke_width=max(1,round(sw*u)),stroke_fill=outer)
    if k=='comicred' and mode=='display':
...
def panel(im,p,kind):
    ...
    if kind=='comicred':
        d.rectangle((0,q/2,out.width,out.height-q/2),fill=(224,31,24,235));d.polygon([(out.width-22*u,out.height-q/2),(out.width-5*u,out.height-q/2),(out.width-15*u,out.height+2*u)],fill=(130,19,20,235))
    elif kind=='romance':
        d.rounded_rectangle((q,q,out.width-q,out.height-q),radius=round(16*u),fill=(255,241,239,205),outline='#E6A8B9',width=max(1,round(2*u)))
        d.line((q*1.5,out.height-q*.8,out.width-q*1.5,out.height-q*.8),fill='#F39DB5',width=max(1,round(u)))
    elif kind=='emojiwhite':
        d.rounded_rectangle((q,q,out.width-q,out.height-q),radius=round(4*u),fill=(10,10,10,215),outline='#FFFFFF',width=max(1,round(2*u)))
    else:
        d.rectangle((q,q,out.width-q,out.height-q),fill=(7,16,20,225),outline='#FF7540',width=max(1,round(2*u)))
        d.line((q,out.height-q*1.4,out.width-q,out.height-q*1.4),fill='#24C5D6',width=max(1,round(u)))
...
def title(p):
    ...
    for i,text in enumerate(p.p.get('scene_title_lines',[])):
        ...
        rr='keyword' if k in ('romance','emojiwhite') and i==0 else 'body' if k in ('romance','emojiwhite') else 'title'
        im=line(p,[dict(text=text,role=rr)],size,'title','display' if k=='comicred' else 'normal',p.w*.84)
        if k=='comicred':im=panel(im,p,'comicred')
        elif k=='romance':im=im.rotate(-2 if i%2==0 else 2,resample=Image.Resampling.BICUBIC,expand=True)
        elif k=='emojiwhite':im=panel(im,p,'emojiwhite')
        elif k=='contrastpop':im=im.rotate(-2 if i==0 else 1,resample=Image.Resampling.BICUBIC,expand=True)
        ims.append(im)
    return stack(ims,round((-2 if k=='emojiwhite' else 2)*u)) if ims else None
...
def label(p,e):
    ...
    if k in ('romance','emojiwhite','contrastpop'):im=panel(im,p,k)
    symbol=e.get('symbol')
    if symbol:
        q=round(42*u);out=Image.new('RGBA',(im.width+q, max(im.height,round(46*u))));out.alpha_composite(im,(q,round((out.height-im.height)/2)));d=ImageDraw.Draw(out);_symbol(d,symbol,q*.48,out.height/2,11*u,{'romance':'#F65FA7','emojiwhite':'#FFD45A','contrastpop':'#FFB35C','comicred':'#F5D35E'}[k],u);im=out
    if k=='comicred':
...
def layout(p):
    ...
    ti=title(p)
    if ti:add(ti,(p.w-ti.width)/2,p.h*c['title_y']-ti.height/2,0,min(p.p['duration'],p.p.get('title_duration',c['title_duration'])),'title','pop' if k in ('romance','contrastpop') else 'fade')
    for g in p.p['scene_captions']:
        blocks=[]
        for e in g['phrases']:
            ...
            if k=='contrastpop' and mode=='arc':im=arc_line(p,runs,size,p.w*.84)
            else:im=line(p,runs,size,'body',mode,p.w*.84)
            if k=='comicred' and mode=='normal':im=panel(im,p,'comicred')
            if k=='emojiwhite' and mode=='hand':
                ...
                for x,y in ((2*u,im.height*.3),(8*u,im.height*.65),(out.width-7*u,im.height*.4)):d.rectangle((x,y,x+4*u,y+4*u),fill='#F59AB8');im=out
            blocks.append((e,im))
        ...
        for i,(e,im) in enumerate(blocks):
            x=(p.w-im.width)/2
            if len(blocks)==2 and k in ('comicred','romance','emojiwhite','contrastpop'):x=p.w*.07 if i==0 else p.w*.93-im.width
            anim='soft' if k=='comicred' else 'pop';add(im,x,y,e['start'],e['end'],'caption',anim);y+=im.height+gap
    for e in p.p.get('scene_tags',[]):

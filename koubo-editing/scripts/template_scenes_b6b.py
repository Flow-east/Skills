"""B6b reference-led yellow/white and three playful designs.
...
KINDS={'floraltravel','magenta','lightbulb','redfestive'}
MODES={'floraltravel':{'normal','display'},'magenta':{'normal','display'},'lightbulb':{'normal','hand'},'redfestive':{'normal','arc'}}
SYMBOLS={'star','heart','arrow','burst','exclaim'}

def validate(p):
    from template_scenes import texts,interval
    k=p.variant['scene_system']['kind'];ids={}
    if any(p.p.get(x) for x in ('scene_notes','scene_highlights','scene_title_surface','scene_identity','scene_accents','scene_media','scene_callouts')):raise ValueError('Unsupported B6b auxiliary layer')
    for g in p.p['scene_captions']:
        for e in g['phrases']:
            ident=e.get('id')
            if not isinstance(ident,str) or not ident or ident in ids:raise ValueError('B6b requires unique nonempty phrase ids')
            ids[ident]=e
            if e.get('mode','normal') not in MODES[k]:raise ValueError('Unsupported B6b phrase mode')
            if any(e.get(x) for x in ('pointer','underline','reveal','reveal_times')) or e.get('color','normal')!='normal' or any('fill' in r for r in e.get('runs',[])):raise ValueError('B6b colors/decorations are contract controlled')
    for e in p.p.get('scene_tags',[]):
        a,b=interval(e,p.p['duration']);q=ids.get(e.get('phrase_id'))
        if not q or not q['start']<=a<b<=q['end'] or e.get('text') not in texts(q):raise ValueError('B6b tag must quote an active phrase')
        if e.get('orientation','horizontal')!='horizontal' or e.get('side','right') not in ('left','right'):raise ValueError('B6b tags are horizontal and side anchored')
        symbol=e.get('symbol')
        if symbol is not None and (symbol not in SYMBOLS or k=='floraltravel'):raise ValueError('Unsupported semantic sticker symbol')
        if symbol and not str(e.get('symbol_reason','')).strip():raise ValueError('Sticker symbol requires explicit semantic reason')
        if any(x in e for x in ('asset','path','image','icon')):raise ValueError('B6b tags use original geometry, not undeclared sticker assets')
        if any(v in e for v in ('x','y')):
            from template_scenes import finite
            if not all(v in e and finite(e[v]) and 0<=e[v]<=1 for v in ('x','y')):raise ValueError('B6b explicit tag anchor needs normalized x/y pair')
...
def _metrics(p,runs,size,role,mode,maxw):
    ...
    for _ in range(5):
        glyphs=[];x=0
        for r in runs:
            rr=r.get('role',role);mult=1.08 if rr=='keyword' and k in ('magenta','lightbulb') else 1
        ...
        if size<initial*.70:raise ValueError('B6b text too long; split semantic phrases rather than shrink')
    raise ValueError('B6b text does not fit')
...
def line(p,runs,size,role='body',mode='normal',maxw=None):
    ...
    for x,text,f,rr,b in glyphs:
        ...
        if k=='floraltravel':
            fill='#F5D35E' if rr=='keyword' else '#FFFFFF';edge='#5A2D1F';sw=1.4 if mode=='display' else .8
        elif k=='magenta':
            fill='#F75C9E' if rr=='keyword' else '#FFF3F6';edge='#7C194B';sw=2.0
        elif k=='lightbulb':
            fill='#F0C34A' if rr=='keyword' else '#FFF7DD';edge='#362A2A';sw=1.6
        else:
            fill={'title':'#FFD58A','keyword':'#FFB35C','body':'#FFF1DA','sticker':'#FFD58A'}.get(rr,'#FFF1DA');edge='#7C261E';sw=2.5
        d.text((pos[0]+1.4*u,pos[1]+2*u),text,font=f,anchor='ls',fill=edge,stroke_width=max(1,round((sw+1.4)*u)),stroke_fill=edge)
        outer='#FFFFFF' if k in ('magenta','lightbulb') else edge
        d.text(pos,text,font=f,anchor='ls',fill=fill,stroke_width=max(1,round(sw*u)),stroke_fill=outer)
    if k=='floraltravel' and mode=='display':
...
def panel(im,p,kind):
    ...
    if kind=='floraltravel':
        d.rectangle((0,q/2,out.width,out.height-q/2),fill=(110,48,28,190))
        d.rectangle((q,q,out.width-q,out.height-q),outline=(255,231,139,230),width=max(1,round(2*u)))
        for x in (q*1.2,out.width-q*1.2): d.line((x,q,x,out.height-q),fill=(255,183,82,220),width=max(1,round(u)))
    elif kind=='magenta':
        d.rounded_rectangle((q,q,out.width-q,out.height-q),radius=round(12*u),fill=(255,235,247,225),outline='#D62F7D',width=max(1,round(2*u)))
        d.line((q*1.3,out.height-q*.8,out.width-q*1.3,out.height-q*.8),fill='#F15CA0',width=max(1,round(u)))
    elif kind=='lightbulb':
        d.rectangle((q,q,out.width-q,out.height-q),fill=(18,20,24,190),outline='#E8D8B8',width=max(1,round(u)))
        d.line((q,out.height-q*1.4,out.width-q,out.height-q*1.4),fill='#BBA98A',width=max(1,round(u)))
    else:
        d.rounded_rectangle((q,q,out.width-q,out.height-q),radius=round(5*u),fill=(255,240,213,235),outline='#7C261E',width=max(1,round(2*u)))
...
def title(p):
    ...
    for i,text in enumerate(p.p.get('scene_title_lines',[])):
        ...
        rr='keyword' if k in ('magenta','lightbulb') and i==0 else 'body' if k in ('magenta','lightbulb') else 'title'
        im=line(p,[dict(text=text,role=rr)],size,'title','display' if k=='floraltravel' else 'normal',p.w*.84)
        if k=='floraltravel':im=panel(im,p,'floraltravel')
        elif k=='magenta':im=im.rotate(-2 if i%2==0 else 2,resample=Image.Resampling.BICUBIC,expand=True)
        elif k=='lightbulb':im=panel(im,p,'lightbulb')
        elif k=='redfestive':im=im.rotate(-2 if i==0 else 1,resample=Image.Resampling.BICUBIC,expand=True)
        ims.append(im)
    return stack(ims,round((-2 if k=='lightbulb' else 2)*u)) if ims else None
...
def label(p,e):
    ...
    if k in ('magenta','lightbulb','redfestive'):im=panel(im,p,k)
    symbol=e.get('symbol')
    if symbol:
        q=round(42*u);out=Image.new('RGBA',(im.width+q, max(im.height,round(46*u))));out.alpha_composite(im,(q,round((out.height-im.height)/2)));d=ImageDraw.Draw(out);_symbol(d,symbol,q*.48,out.height/2,11*u,{'magenta':'#F65FA7','lightbulb':'#FFD45A','redfestive':'#FFB35C','floraltravel':'#F5D35E'}[k],u);im=out
    if k=='floraltravel':
...
def layout(p):
    ...
    ti=title(p)
    if ti:add(ti,(p.w-ti.width)/2,p.h*c['title_y']-ti.height/2,0,min(p.p['duration'],p.p.get('title_duration',c['title_duration'])),'title','pop' if k in ('magenta','redfestive') else 'fade')
    for g in p.p['scene_captions']:
        blocks=[]
        for e in g['phrases']:
            ...
            if k=='redfestive' and mode=='arc':im=arc_line(p,runs,size,p.w*.84)
            else:im=line(p,runs,size,'body',mode,p.w*.84)
            if k=='floraltravel' and mode=='normal':im=panel(im,p,'floraltravel')
            if k=='lightbulb' and mode=='hand':
                ...
                for x,y in ((2*u,im.height*.3),(8*u,im.height*.65),(out.width-7*u,im.height*.4)):d.rectangle((x,y,x+4*u,y+4*u),fill='#F65FA7');im=out
            blocks.append((e,im))
        ...
        for i,(e,im) in enumerate(blocks):
            x=(p.w-im.width)/2
            if len(blocks)==2 and k in ('floraltravel','magenta','lightbulb','redfestive'):x=p.w*.07 if i==0 else p.w*.93-im.width
            anim='soft' if k=='floraltravel' else 'pop';add(im,x,y,e['start'],e['end'],'caption',anim);y+=im.height+gap
    for e in p.p.get('scene_tags',[]):

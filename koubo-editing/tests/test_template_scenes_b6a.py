...
from template_scenes_b6a import KINDS,MODES,title,line,label,panel,arc_line
...
class B6aTests(unittest.TestCase):
 ...
 def test_ids_required_unique(self):
  p=fixture('waxcute');p['scene_captions'][0]['phrases'][0].pop('id')
  with self.assertRaises(ValueError):painter(p)
  p=fixture('waxcute');q=copy.deepcopy(p['scene_captions'][0]['phrases'][0]);p['scene_captions'][0]['phrases'].append(q)
  with self.assertRaises(ValueError):painter(p)
 def test_cleanwhite_single_phrase(self):
  p=fixture('cleanwhite');q=copy.deepcopy(p['scene_captions'][0]['phrases'][0]);q['id']='p1';p['scene_captions'][0]['phrases'].append(q)
  with self.assertRaises(ValueError):painter(p)
 def test_playful_dual_phrases(self):
  for k in ('waxcute','qqcute','energycartoon'):
   p=fixture(k);a=p['scene_captions'][0]['phrases'][0];a.update(text='方法',runs=[dict(text='方法')]);q=copy.deepcopy(a);q.update(id='p1',text='清楚',runs=[dict(text='清楚',role='keyword')],start=1.2);p['scene_captions'][0]['phrases'].append(q);x=painter(p);self.assertTrue(audit_layout(x)['passed']);es=[e for e in layout(x) if e['role']=='caption'];self.assertEqual(len(es),2);self.assertNotEqual(es[0]['x'],es[1]['x'])
 def test_no_color_or_reveal_override(self):
  for extra in ({'color':'red'},{'pointer':'x'},{'reveal_times':[.2]}):
   p=fixture('qqcute');p['scene_captions'][0]['phrases'][0].update(extra)
   with self.assertRaises(ValueError):painter(p)
 def test_no_fill_override(self):
  p=fixture('energycartoon');p['scene_captions'][0]['phrases'][0]['runs'][0]['fill']='#f00'
  with self.assertRaises(ValueError):painter(p)
 def test_tags_quote_and_clock(self):
  p=fixture('waxcute');p['scene_tags']=[dict(tag('star'),text='促销')]
  with self.assertRaises(ValueError):painter(p)
  p=fixture('waxcute');p['scene_tags']=[dict(tag('star'),start=0)]
  with self.assertRaises(ValueError):painter(p)
 def test_symbol_requires_reason(self):
  p=fixture('waxcute');e=tag('star');e.pop('symbol_reason');p['scene_tags']=[e]
  with self.assertRaises(ValueError):painter(p)
 def test_cleanwhite_no_symbol(self):
  p=fixture('cleanwhite');p['scene_tags']=[tag('star')]
  with self.assertRaises(ValueError):painter(p)
 def test_symbols_supported(self):
  for k,symbol in (('waxcute','star'),('qqcute','exclaim'),('energycartoon','heart'),('energycartoon','arrow'),('energycartoon','burst')):
   p=fixture(k);p['scene_tags']=[tag(symbol)];self.assertTrue(audit_layout(painter(p))['passed'])
 def test_explicit_tag_anchor_pair(self):
  p=fixture('waxcute');e=tag('star');e['x']=.1;p['scene_tags']=[e]
  with self.assertRaises(ValueError):painter(p)
  e['y']=.5;self.assertTrue(audit_layout(painter(p))['passed'])
 def test_no_external_sticker_asset(self):
  p=fixture('qqcute');e=tag('exclaim');e['path']='sticker.png';p['scene_tags']=[e]
  with self.assertRaises(ValueError):painter(p)
 def test_no_unsupported_layers(self):
  for key in ('scene_media','scene_accents','scene_identity','scene_callouts'):
   p=fixture('waxcute');p[key]=[dict(text='x')]
   with self.assertRaises(ValueError):painter(p)
 def test_protected_region_blocks_tag(self):
  p=fixture('energycartoon');p['scene_tags']=[tag('arrow')];p['protected_regions']=[dict(box=[0,0,1,1])]
 ...
 def test_arc_shrinks_then_rejects_extreme(self):
  a=painter(fixture('energycartoon'));im=arc_line(a,[dict(text='检查工作流程',role='body')],50,620);self.assertLessEqual(im.width,620)
 ...
 def test_panel_alpha_and_stability(self):
  ...
  for k in ('cleanwhite','waxcute','qqcute','energycartoon'):
   a=painter(fixture(k));x=panel(src,a,k);y=panel(src,a,k);self.assertEqual(x.tobytes(),y.tobytes());self.assertGreater(x.getchannel('A').getbbox()[2],0)
 def test_labels_distinct(self):
  hs=[]
  for k,s in (('cleanwhite',None),('waxcute','star'),('qqcute','exclaim'),('energycartoon','arrow')):
 ...
 def test_transcript_integrity(self):
  p=fixture('cleanwhite');p['scene_captions'][0]['phrases'][0]['runs']=[dict(text='错误内容')]
 ...
 def test_energy_vignette_only(self):
  p=fixture('waxcute');p['scene_canvas']=[dict(kind='vignette',start=0,end=4,strength=.2,reason='test')]
  with self.assertRaises(ValueError):painter(p)
  p=fixture('energycartoon');p['scene_canvas']=[dict(kind='vignette',start=0,end=4,strength=.2,reason='synthetic only')];self.assertTrue(audit_layout(painter(p))['passed'])

...
from template_scenes_b6c import KINDS,MODES,title,line,label,panel,arc_line
...
class B6cTests(unittest.TestCase):
 ...
 def test_ids_required_unique(self):
  p=fixture('romance');p['scene_captions'][0]['phrases'][0].pop('id')
  with self.assertRaises(ValueError):painter(p)
  p=fixture('romance');q=copy.deepcopy(p['scene_captions'][0]['phrases'][0]);p['scene_captions'][0]['phrases'].append(q)
  with self.assertRaises(ValueError):painter(p)
 ...
 def test_no_color_reveal_or_fill_override(self):
  for extra in ({'color':'red'},{'pointer':'x'},{'reveal_times':[.2]}):
   p=fixture('romance');p['scene_captions'][0]['phrases'][0].update(extra)
   with self.assertRaises(ValueError):painter(p)
  p=fixture('contrastpop');p['scene_captions'][0]['phrases'][0]['runs'][0]['fill']='#f00'
  with self.assertRaises(ValueError):painter(p)
 def test_tags_quote_and_clock(self):
  p=fixture('romance');p['scene_tags']=[dict(tag('heart'),text='促销')]
  with self.assertRaises(ValueError):painter(p)
  p=fixture('romance');p['scene_tags']=[dict(tag('heart'),start=0)]
  with self.assertRaises(ValueError):painter(p)
 def test_symbol_requires_reason(self):
  p=fixture('romance');e=tag('heart');e.pop('symbol_reason');p['scene_tags']=[e]
  with self.assertRaises(ValueError):painter(p)
 def test_comicred_no_symbol(self):
  p=fixture('comicred');p['scene_tags']=[tag('star')]
  with self.assertRaises(ValueError):painter(p)
 def test_symbols_supported(self):
  for k,symbol in (('romance','heart'),('emojiwhite','star'),('emojiwhite','exclaim'),('contrastpop','burst'),('contrastpop','arrow')):
   p=fixture(k);p['scene_tags']=[tag(symbol)];self.assertTrue(audit_layout(painter(p))['passed'])
 def test_explicit_tag_anchor_pair(self):
  p=fixture('emojiwhite');e=tag('star');e['x']=.1;p['scene_tags']=[e]
  with self.assertRaises(ValueError):painter(p)
  e['y']=.5;self.assertTrue(audit_layout(painter(p))['passed'])
 def test_no_external_sticker_asset(self):
  p=fixture('contrastpop');e=tag('burst');e['path']='sticker.png';p['scene_tags']=[e]
  with self.assertRaises(ValueError):painter(p)
 def test_no_unsupported_layers(self):
  for key in ('scene_media','scene_accents','scene_identity','scene_callouts'):
   p=fixture('romance');p[key]=[dict(text='x')]
   with self.assertRaises(ValueError):painter(p)
 def test_protected_region_blocks_tag(self):
  p=fixture('contrastpop');p['scene_tags']=[tag('arrow')];p['protected_regions']=[dict(box=[0,0,1,1])]
 ...
 def test_arc_shrinks_then_rejects_extreme(self):
  a=painter(fixture('contrastpop'));im=arc_line(a,[dict(text='检查工作流程',role='body')],50,620);self.assertLessEqual(im.width,620)
 ...
 def test_labels_distinct(self):
  hs=[]
  for k,s in (('comicred',None),('romance','heart'),('emojiwhite','star'),('contrastpop','burst')):
 ...
 def test_transcript_integrity(self):
  p=fixture('comicred');p['scene_captions'][0]['phrases'][0]['runs']=[dict(text='错误内容')]
